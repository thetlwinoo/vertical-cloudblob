...
/**
 * Performance test for the Images entity.
 */
class ImagesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Images entity")
        ...
        .repeat(2) {
            ...
            .get("/services/cloudblob/api/images")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new images")
            .post("/services/cloudblob/api/images")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "thumbnail":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_images_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created images")
                .get("/services/cloudblob${new_images_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created images")
            .delete("/services/cloudblob${new_images_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
